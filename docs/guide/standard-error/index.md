...
## Inline Syntax on `stderr`

If you have the `inline` feature enabled, inline syntax works on stderr too, with no separate macros needed.

```rust
use farben::prelude::*;

ceprintln!("*error:* the config file is /missing/ or malformed.");
```

...
fn parse_part(part: &str) -> TagType {
    if part == "/" {
        TagType::Reset
    } else if let Some(color) = NamedColor::from_str(part) {
        TagType::Color(Color::Named(color))
        ...
        match ansi_val.trim().parse::<u8>() {
            Ok(code) => TagType::Color(Color::Ansi256(code)),
            Err(_) => todo!(),
        }
        ...
        match parts {
            Ok(v) if v.len() == 3 => TagType::Color(Color::Rgb(v[0], v[1], v[2])),
            _ => todo!(),
        }
    } else {
        todo!()
...
fn parse_tag(raw_tag: &str) -> Vec<TagType> {
    raw_tag.split_whitespace().map(parse_part).collect()
}

pub(crate) fn tokenize(input: impl Into<String>) -> Vec<Token> {
    ...
    loop {
        ...
        let Some(closing) = input[abs_starting..].find(']') else {
            tokens.push(Token::Text(input[abs_starting..].to_string()));
            break;
        };
